...
Mirrors the _defualt file but with added async functionality.
"""
...
def load_credentials_from_file(filename, scopes=None):
    ...
    Returns:
        Tuple[google.auth.credentials_async.Credentials, Optional[str]]: Loaded
    ...
    if credential_type == _AUTHORIZED_USER_TYPE:
        ...
        try:
            credentials_async = credentials_async.Credentials.from_authorized_user_info(
                info, scopes=scopes
            )
        except ValueError as caught_exc:
            ...
            six.raise_from(new_exc, caught_exc)
        if not credentials_async.quota_project_id:
            _warn_about_problematic_credentials(credentials_async)
        return credentials_async, None
...
def default_async(scopes=None, request=None):
