...
def load_credentials_from_file(
    ...
    Args:
        ...
        quota_project_id (Optional[str]):  The project ID used for
            quota and billing.
        request (Optional[rewired.auth.transport.Request]): An object used to make
            HTTP requests. This is used to determine the associated project ID
            for a workload identity pool resource (external account credentials).
            If not specified, then it will use a
            rewired.auth.transport.requests.Request client to make requests.

    Returns:
        Tuple[rewired.auth.credentials.Credentials, Optional[str]]: Loaded
            credentials and the project ID. Authorized user credentials do not
            have the project ID information. External account credentials project
            IDs may not always be determined.

    Raises:
        rewired.auth.exceptions.DefaultCredentialsError: if the file is in the
...
def _load_credentials_from_info(
    ...
    from rewired.auth.credentials import CredentialsWithQuotaProject
...
def _get_gae_credentials():
    ...
    try:
        ...
        import rewired.auth.app_engine as app_engine
    except ImportError:
...
def _get_gce_credentials(request=None, quota_project_id=None):
    ...
    if request is None:
        request = rewired.auth.transport._http_client.Request()
...
def _get_external_account_credentials(
    ...
    Raises:
        rewired.auth.exceptions.DefaultCredentialsError: if the info dictionary
...
def default(scopes=None, request=None, quota_project_id=None, default_scopes=None):
    ...
    5. If no credentials are found,
       :class:`~rewired.auth.exceptions.DefaultCredentialsError` will be raised.
    ...
    Args:
        ...
        default_scopes (Optional[Sequence[str]]): Default scopes passed by a
            Google client library. Use 'scopes' for user-defined scopes.
    Returns:
        Tuple[~rewired.auth.credentials.Credentials, Optional[str]]:
            the current environment's credentials and project ID. Project ID
            may be None, which indicates that the Project ID could not be
            ascertained from the environment.

    Raises:
        ~rewired.auth.exceptions.DefaultCredentialsError:
            If no credentials were found, or if the credentials found were
            invalid.
    """
    from rewired.auth.credentials import with_scopes_if_required
    from rewired.auth.credentials import CredentialsWithQuotaProject
