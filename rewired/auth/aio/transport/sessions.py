...
from rewired.auth import _exponential_backoff, exceptions
from rewired.auth.aio import transport
from rewired.auth.aio.credentials import Credentials
from rewired.auth.exceptions import TimeoutError

try:
    from rewired.auth.aio.transport.aiohttp import Request as AiohttpRequest
...
async def timeout_guard(timeout):
    ...
    Raises:
        rewired.auth.exceptions.TimeoutError: If the code within the context exceeds the provided timeout.
...
class AsyncAuthorizedSession:
    """This is an asynchronous implementation of :class:`rewired.auth.requests.AuthorizedSession` class.
    We utilize an instance of a class that implements :class:`rewired.auth.aio.transport.Request` configured
    by the caller or otherwise default to `rewired.auth.aio.transport.aiohttp.Request` if the external aiohttp
    ...
    authorization::
        ...
        import aiohttp
        from rewired.auth.aio.transport import sessions
    ...
    Args:
        credentials (rewired.auth.aio.credentials.Credentials):
            The credentials to add to the request.
        auth_request (Optional[rewired.auth.aio.transport.Request]):
            An instance of a class that implements
            :class:`~rewired.auth.aio.transport.Request` used to make requests
            and refresh credentials. If not passed,
            an instance of :class:`~rewired.auth.aio.transport.aiohttp.Request`
            is created.

    Raises:
        - rewired.auth.exceptions.TransportError: If `auth_request` is `None`
            and the external package `aiohttp` is not installed.
        - rewired.auth.exceptions.InvalidType: If the provided credentials are
            not of type `rewired.auth.aio.credentials.Credentials`.
    """
    ...
    async def request(
        ...
        **kwargs,
        ...
        Returns:
                rewired.auth.aio.transport.Response: The HTTP response.

        Raises:
                rewired.auth.exceptions.TimeoutError: If the method does not complete within
