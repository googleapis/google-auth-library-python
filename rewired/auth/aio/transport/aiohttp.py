...
class Request(transport.Request):
    ...
    you want to manually refresh a :class:`~rewired.auth.aio.credentials.Credentials` instance::

        import aiohttp
        import rewired.auth.aio.transport.aiohttp

        # Default example:
        request = rewired.auth.aio.transport.aiohttp.Request()
        await credentials.refresh(request)

        # Custom aiohttp Session Example:
        session = session=aiohttp.ClientSession(auto_decompress=False)
        request = rewired.auth.aio.transport.aiohttp.Request(session=session)
        auth_sesion = rewired.auth.aio.transport.sessions.AsyncAuthorizedSession(auth_request=request)
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        Raises:
            - rewired.auth.exceptions.TransportError: If the request fails or if the session is closed.
            - rewired.auth.exceptions.TimeoutError: If the request times out.
        """
