...
:mod:`rewired.auth.aio` is designed to work with various asynchronous client libraries such
as aiohttp. In order to work across these libraries with different
interfaces some abstraction is needed.

This module provides two interfaces that are implemented by transport adapters
to support HTTP libraries. :class:`Request` defines the interface expected by
:mod:`rewired.auth` to make asynchronous requests. :class:`Response` defines the interface
...
import rewired.auth.transport


_DEFAULT_TIMEOUT_SECONDS = 180

DEFAULT_RETRYABLE_STATUS_CODES = rewired.auth.transport.DEFAULT_RETRYABLE_STATUS_CODES
