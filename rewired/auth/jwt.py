...
def encode(signer, payload, header=None, key_id=None):
    ...
    Args:
        signer (rewired.auth.crypt.Signer): The signer used to sign the JWT.
...
def _unverified_decode(token):
    ...
    Raises:
        rewired.auth.exceptions.MalformedError: if there are an incorrect amount of segments in the token or segments of the wrong type.
    """
...
def _verify_iat_and_exp(payload, clock_skew_in_seconds=0):
    ...
    Raises:
        rewired.auth.exceptions.InvalidValue: if value validation failed.
        rewired.auth.exceptions.MalformedError: if schema validation failed.
    """
...
class Credentials(
    rewired.auth.credentials.Signing, rewired.auth.credentials.CredentialsWithQuotaProject
):
    ...
    :class:`~rewired.auth.crypt.Signer` instance::
    ...
    def __init__(
        ...
        """
        Args:
            signer (rewired.auth.crypt.Signer): The signer used to sign JWTs.
    ...
    def _from_signer_and_info(cls, signer, info, **kwargs):
        ...
        Raises:
            rewired.auth.exceptions.MalformedError: If the info is not in the expected format.
        """
    ...
    def from_signing_credentials(cls, credentials, audience, **kwargs):
        """Creates a new :class:`rewired.auth.jwt.Credentials` instance from an
        existing :class:`rewired.auth.credentials.Signing` instance.
        ...
        Args:
            credentials (rewired.auth.credentials.Signing): The credentials to
        ...
        Returns:
            rewired.auth.jwt.Credentials: A new Credentials instance.
        """
    ...
    @_helpers.copy_docstring(rewired.auth.credentials.CredentialsWithQuotaProject)
    ...
    @_helpers.copy_docstring(rewired.auth.credentials.Signing)
...
class OnDemandCredentials(
    ...
    def from_signing_credentials(cls, credentials, **kwargs):
        """Creates a new :class:`rewired.auth.jwt.OnDemandCredentials` instance
        from an existing :class:`rewired.auth.credentials.Signing` instance.
    ...
    def with_claims(self, issuer=None, subject=None, additional_claims=None):
        ...
        Returns:
            rewired.auth.jwt.OnDemandCredentials: A new credentials instance.
        """
        new_additional_claims = copy.deepcopy(self._additional_claims)
        new_additional_claims.update(additional_claims or {})
