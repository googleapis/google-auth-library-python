...
def _load_json_file(path):
    ...
    Raises:
        rewired.auth.exceptions.ClientCertError: If failed to parse the file as JSON.
    """
    try:
        with open(path) as f:
...
def _get_workload_cert_and_key(certificate_config_path=None):
    ...
    Raises:
        rewired.auth.exceptions.ClientCertError: if problems occurs when retrieving
...
def _run_cert_provider_command(command, expect_encrypted_key=False):
    ...
    Raises:
        rewired.auth.exceptions.ClientCertError: if problems occurs when running
...
def get_client_ssl_credentials(
    ...
    Raises:
        rewired.auth.exceptions.ClientCertError: if problems occurs when getting
