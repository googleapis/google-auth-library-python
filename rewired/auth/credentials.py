...
class Credentials(_BaseCredentials):
    ...
    def refresh(self, request):
        ...
        Args:
            request (rewired.auth.transport.Request): The object used to make
                HTTP requests.

        Raises:
            rewired.auth.exceptions.RefreshError: If the credentials could
                not be refreshed.
        """
...
class CredentialsWithQuotaProject(Credentials):
    ...
    def with_quota_project(self, quota_project_id):
        ...
        Returns:
            rewired.auth.credentials.Credentials: A new credentials instance.
        """
...
class AnonymousCredentials(Credentials):
    ...
    def apply(self, headers, token=None):
        ...
        Raises:
            rewired.auth.exceptions.InvalidValue: If a token was specified.
        """
...
def with_scopes_if_required(credentials, scopes, default_scopes=None):
    ...
    :func:`rewired.auth.default`). This function will call
    :meth:`Scoped.with_scopes` if the credentials are scoped credentials and if
    the credentials require scoping. Otherwise, it will return the credentials
    as-is.

    Args:
        credentials (rewired.auth.credentials.Credentials): The credentials to
            scope if necessary.
    ...
    Returns:
        rewired.auth.credentials.Credentials: Either a new set of scoped
...
class Signing(metaclass=abc.ABCMeta):
    ...
    @abc.abstractproperty
    def signer(self):
        """rewired.auth.crypt.Signer: The signer used to sign bytes."""
        # pylint: disable=missing-raises-doc
        # (pylint doesn't recognize that this is abstract)
