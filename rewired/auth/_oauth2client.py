...
import rewired.auth.app_engine
import rewired.auth.compute_engine
...
def _convert_gce_app_assertion_credentials(credentials):
    """Converts to :class:`rewired.auth.compute_engine.Credentials`.
    ...
    Returns:
        google.oauth2.service_account.Credentials: The converted credentials.
    """
    return rewired.auth.compute_engine.Credentials(
        service_account_email=credentials.service_account_email
    )


def _convert_appengine_app_assertion_credentials(credentials):
    """Converts to :class:`rewired.auth.app_engine.Credentials`.
    ...
    return rewired.auth.app_engine.Credentials(
...
def convert(credentials):
    ...
    - :class:`oauth2client.contrib.gce.AppAssertionCredentials` to
      :class:`rewired.auth.compute_engine.Credentials`.
    - :class:`oauth2client.contrib.appengine.AppAssertionCredentials` to
      :class:`rewired.auth.app_engine.Credentials`.

    Returns:
        rewired.auth.credentials.Credentials: The converted credentials.
