...
def test_byoid_metric_header(mock_python_and_auth_lib_version):
    metrics_options = {}
    assert(
        metrics.byoid_metrics_header(metrics_options) == "gl-python/3.7 auth/1.1 google-byoid-sdk"
    )
    metrics_options["testKey"] = "testValue"
    assert(
        metrics.byoid_metrics_header(metrics_options) == "gl-python/3.7 auth/1.1 google-byoid-sdk testKey/testValue"
    )
