...
class TestCredentials(object):
    ...
    VALID_REGIONAL_ACCESS_BOUNDARY = {"encodedLocations": "valid-encoded-locations"}
    EXPECTED_REGIONAL_ACCESS_BOUNDARY_LOOKUP_URL_DEFAULT_UNIVERSE = "https://iamcredentials.googleapis.com/v1/projects/-/serviceAccounts/default/allowedLocations"
    ...
    def test_with_regional_access_boundary(self):
        ...
        new_creds = creds.with_regional_access_boundary(new_boundary)

        assert new_creds is not creds
        assert new_creds._regional_access_boundary == new_boundary
    ...
    @mock.patch("google.oauth2._client._lookup_regional_access_boundary", autospec=True)
    @mock.patch("google.auth.compute_engine._metadata.get", autospec=True)
    def test_refresh_regional_access_boundary_lookup_skipped_if_env_var_not_true(
        self, mock_metadata_get, mock_lookup_rab
    ):
        ...
        with mock.patch.dict(
            os.environ,
            {
                environment_vars.GOOGLE_AUTH_REGIONAL_ACCESS_BOUNDARY_ENABLE_EXPERIMENT: "false"
            },
        ):
    ...
    @mock.patch("google.oauth2._client._lookup_regional_access_boundary", autospec=True)
    @mock.patch("google.auth.compute_engine._metadata.get", autospec=True)
    def test_refresh_regional_access_boundary_lookup_skipped_if_env_var_missing(
        self, mock_metadata_get, mock_lookup_rab
    ):
        creds = self.credentials
        request = mock.Mock()
    ...
    def test_build_regional_access_boundary_lookup_url_non_default_universe(
        self, mock_get_universe_domain, mock_get_service_account_info
    ):
    ...
    def test_build_regional_access_boundary_lookup_url_get_service_account_info_fails(
        ...
        with pytest.raises(exceptions.RefreshError):
            creds._build_regional_access_boundary_lookup_url()
