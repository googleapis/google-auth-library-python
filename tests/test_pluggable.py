...
def test_from_constructor_and_injection(self):
    ...
    assert "Missing credential_source" in str(excinfo.value)
    ...
    assert "Executables need to be explicitly allowed" in str(excinfo.value)
    ...
    assert "Executable returned unsupported version." in str(excinfo.value)
    ...
    assert "The token returned by the executable is expired." in str(excinfo.value)
    ...
    assert "The executable response is missing the version field." in str(excinfo.value)
    ...
    assert "Executable returned unsupported token type." in str(excinfo.value)
...
def test_retrieve_subject_token_missing_token_type(self):
    ...
    assert "Timeout must be between 5 and 120 seconds." in str(excinfo.value)
    ...
    assert "Interactive mode is only enabled for workforce pool." in str(excinfo.value)
...
with pytest.raises(ValueError) as excinfo:
    ...
    assert str(data.get("expectErrPattern") in str(excinfo.value)
    ...
    assert "Pluggable auth is only supported for python 3.7+" in str(excinfo.value)
