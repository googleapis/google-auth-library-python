...
def test_refresh_source_credentials(self, time_skew):
    ...
    assert str(impersonated_credentials._REFRESH_ERROR) in str(excinfo.value)
    ...
    assert "Error getting ID token" in str(excinfo.value)
...
def test_sign_bytes_failure(self):
    ...
    assert "'code': 403" in str(excinfo.value)
    ...
    auth_session.return_value = mock_response
            ...
            with pytest.raises(exceptions.TransportError) as excinfo:
    credentials.sign_bytes(b"foo")
    assert "exhausted signBlob endpoint retries" in str(excinfo.value)
...
with pytest.raises(exceptions.GoogleAuthError) as excinfo:
    ...
    assert str("Provided Credential must be" " impersonated_credentials") in str(excinfo.value)
