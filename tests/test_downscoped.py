...
def make_credential_access_boundary(rules):
    ...
    assert "The provided expression is not a string." in str(excinfo.value)

                            def test_invalid_title_type(self):
                                with pytest.raises(TypeError) as excinfo:
    make_availability_condition(EXPRESSION, False, DESCRIPTION)

    assert "The provided title is not a string or None." in str(excinfo.value)

                                    def test_invalid_description_type(self):
                                        with pytest.raises(TypeError) as excinfo:
    make_availability_condition(EXPRESSION, TITLE, False)

    assert "The provided description is not a string or None." in str(excinfo.value)
    ...
    assert "The provided available_resource is not a string." in str(excinfo.value)
    ...
    assert "available_permissions must be prefixed with 'inRole:'." in str(excinfo.value)
...
for (k, v) in body_tuples:
    ...
    assert "Failed to refresh access token in source credentials." in str(excinfo.value)
    assert not credentials.expired
    assert credentials.token is None
