...
class TestAuthorizedSession(object):
    ...
    def test_cert_rotation_when_cert_mismatch(self):
        credentials = mock.Mock(wraps=CredentialsStub())
        final_response = make_response(status=http_client.OK)
        # First request will 401, second request will succeed.
        adapter = AdapterStub(
        ...
        authed_session._cached_cert = lambda: (old_cert)
    ...
    def test_no_cert_rotation_when_cert_match_and_mTLS_enabled(self):
        ...
        adapter = AdapterStub([make_response(status=http_client.UNAUTHORIZED), make_response(status=http_client.UNAUTHORIZED), make_response(status=http_client.UNAUTHORIZED)])
        authed_session = google.auth.transport.requests.AuthorizedSession(
            credentials, refresh_timeout=60
        )
        authed_session.mount(self.TEST_URL, adapter)
    ...
    def test_no_cert_match_check_when_mtls_disabled(self):
        ...
        # Set _is_mtls to False to disable mTLS.
