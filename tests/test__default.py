...
from rewired.auth import _default
...
from rewired.auth import identity_pool
...
with open(AUTHORIZED_USER_FILE) as fh:
    ...
    "rewired.auth._default.load_credentials_from_file",
    ...
    assert "dict type was expected" in str(excinfo.value)
    ...
    assert "not a valid json file" in str(excinfo.value)
    ...
    assert "does not have a valid type" in str(excinfo.value)
    ...
    assert "Failed to load authorized user" in str(excinfo.value)
    ...
    assert "Failed to load service account" in str(excinfo.value)
    assert "missing fields" in str(excinfo.value)
    ...
    assert "Cannot extract target principal" in str(excinfo.value)
    ...
    assert "source credential of type external_account is not supported" in str(excinfo.value)
...
def test__get_explicit_environ_credentials_no_env():
    ...
    @mock.patch("rewired.auth._default._get_gcloud_sdk_credentials", autospec=True)
...
def test__get_gcloud_sdk_credentials(get_adc_path, load, quota_project_id):
    ...
    assert "Failed to load GDCH service account credentials" in str(excinfo.value)
    ...
    @mock.patch(
    "rewired.auth._default._get_explicit_environ_credentials",
    return_value=(MOCK_CREDENTIALS, mock.sentinel.project_id)
    autospec=True,
    )
    ...
    @mock.patch("os.path.isfile", return_value=True, autospec=True)
    @mock.patch(
    "rewired.auth._cloud_sdk.get_application_default_credentials_path",
...
def test_default_explict_project_id(unused_get, monkeypatch):
    ...
    @mock.patch(
    "rewired.auth._default._get_gcloud_sdk_credentials",
    return_value=(MOCK_CREDENTIALS, None)
    autospec=True,
    )
    @mock.patch(
    "rewired.auth._default._get_gae_credentials",
    return_value=(MOCK_CREDENTIALS, None)
    autospec=True,
    )
    @mock.patch(
    "rewired.auth._default._get_gce_credentials",
    return_value=(MOCK_CREDENTIALS, None)
    autospec=True,
    )
...
def test_default_fail(unused_gce, unused_gae, unused_sdk, unused_explicit):
    ...
    assert str(_default._CLOUD_SDK_MISSING_CREDENTIALS) in str(excinfo.value)
    ...
    @mock.patch(
    "rewired.auth.credentials.with_scopes_if_required",
